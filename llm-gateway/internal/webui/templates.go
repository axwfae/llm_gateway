...
<div class="card">
    <table><thead><tr>
        <th>模型名稱 Model Name</th><th>關聯服務器 Server</th><th>模型 ID Model ID</th><th>操作 Actions</th>
    </tr></thead>
...
function loadServerModels(){fetch('/api/server-models').then(r=>r.json()).then(d=>{
    document.getElementById('modelsTable').innerHTML=(d.server_models||[]).map(m=>{
        var sn=servers.find(s=>s.id==m.server_id);return '<tr><td>'+escapeHtml(m.model_name)+'</td><td>'+(sn?escapeHtml(sn.name):m.server_id)+'</td><td>'+escapeHtml(m.model_id)+'</td><td class=actions><button class=small onclick="editModel(\''+m.id+'\')">編輯 Edit</button><button class="small danger" onclick="deleteModel(\''+m.id+'\')">刪除 Delete</button></td></tr>'
    }).join('')
})}
...
loadServersList();loadServerModels();
</script>
`
...
<div class="card">
    <table><thead><tr>
        <th>服務器 Server</th><th>模式 Mode</th><th>API Key</th><th>狀態 Status</th><th>備註 Notes</th><th>權重 Weight</th><th>待選池 Pool</th><th>操作 Actions</th>
    </tr></thead>
...
function loadAPIKeys(){fetch('/api/server-api-keys').then(r=>r.json()).then(d=>{
    document.getElementById('keysTable').innerHTML=(d.server_api_keys||[]).map(k=>{
        ...
        return '<tr><td>'+(sn?escapeHtml(sn.name):k.server_id)+'</td><td>'+modeText(k.status)+'</td><td>'+escapeHtml(k.api_key||'')+'</td><td>'+ledHtml(k.last_check_result)+statusText(k.last_check_result)+(k.last_check_duration?' <span style="color:#7f8c8d;font-size:11px">'+escapeHtml(k.last_check_duration)+'</span>':'')+'</td><td>'+escapeHtml(k.notes||'')+'</td><td>'+(k.negative_weight||0)+'</td><td>'+(k.pending_pool?'<span class="badge badge-blue">Pool</span>':(k.status=='disabled'?'<span class="badge badge-gray">已停用</span>':'<span class="badge badge-yellow">待測試</span>'))+'</td><td class=actions><button class=small onclick="editKey(\''+k.id+'\')">編輯 Edit</button><button class="small danger" onclick="deleteKey(\''+k.id+'\')">刪除 Del</button></td></tr>'
    }).join('')
})}
function addAPIKey(){
