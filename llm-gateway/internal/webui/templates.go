...
<div class="card">
    ...
    <div class="toolbar" style="margin-top:10px">
        <select id="poolTestServerSelect" class="server-select" onchange="loadPoolTestModels()"><option value="">選擇服務器 Select Server</option></select>
        <select id="poolTestModelSelect" class="server-select"><option value="">自動選擇模型 Auto Model</option></select>
...
function loadPoolTestModels(){var sid=document.getElementById('poolTestServerSelect').value;var sel=document.getElementById('poolTestModelSelect');sel.innerHTML='<option value="">自動選擇模型 Auto Model</option>';if(!sid)return;fetch('/api/server-models').then(r=>r.json()).then(function(d){(d.server_models||[]).forEach(function(m){if(m.server_id==sid){sel.innerHTML+='<option value="'+escapeHtml(m.model_id)+'">'+escapeHtml(m.model_name)+'</option>'}})})}
function testSinglePoolKey(id,btn){btn.disabled=true;btn.textContent='測試中...';var mid=document.getElementById('poolTestModelSelect').value;fetch('/api/test-key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key_id:id,model_id:mid||undefined})}).then(function(){btn.disabled=false;btn.textContent='重新測試 Retest';loadPool();loadHistory()})}
function testAllPoolKeys(){var sid=document.getElementById('poolTestServerSelect').value;if(!sid){alert('請選擇服務器 Please select a server');return}
    var btn=document.querySelector('#poolTestServerSelect ~ button');if(!btn)return;btn.disabled=true;btn.textContent='測試中...';
    var mid=document.getElementById('poolTestModelSelect').value;var url='/api/test-key?server_id='+sid+(mid?'&model_id='+encodeURIComponent(mid):'');
    fetch(url).then(function(){btn.disabled=false;btn.textContent='測試所選服務器所有 Key Test All';loadPool();loadHistory()})
}
