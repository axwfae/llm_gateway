...
func handleChatCompletions(c *gin.Context) {
	...
	case "ollama":
		chatPath = "/v1/chat/completions" // ollama uses same path
	default:
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if *debugMode {
			log.Printf("[UPSTREAM_REQUEST] model=%s server=%s url=%s/api/chat", reqBody.Model, info.TargetURL)
		}
...
func checkAllAPIKeys() {
	...
	for _, sv := range servers {
		...
		for _, key := range keys {
			...
			start := time.Now()
			result := testSingleKey(sv.APIURL, key.APIKey, sv.APIType)
			duration := time.Since(start)
