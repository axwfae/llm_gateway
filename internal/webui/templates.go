...
function loadKeys() {
    fetch('/api/server-api-keys').then(r=>r.json()).then(d=>{
        ...
        fetch('/api/servers').then(r=>r.json()).then(s=>{
            const servers = {}; s.servers.forEach(x=>servers[x.id]=x.name);
            tbody.innerHTML = d.server_api_keys.map(k=>'<tr><td>'+(servers[k.server_id]||'')+'</td><td>'+k.api_key+'</td><td>'+(k.negative_weight||0)+'</td><td class="actions"><button class="delete" onclick="deleteAPIKey(\''+k.id+'\')">刪除 Delete</button></td></tr>').join('');
        });
    });
}
function addAPIKey() {
